...
import { Aviso, Buscador, Card, Casilla, KPI, Money, TiraTotales, claseControl } from "./ui";
import {
  POR_HERMANA,
  agruparSinVincular,
  etiquetaMotivo,
 ...
 */
export default function VincularClientes({ liquidados, empresas, cargando, anio, periodo, recargar, onVolver }) {
  ...
  // Qué se está mirando, para los textos. Llega hecho desde el tablero, que es
  // quien decide si se está viendo un año o todo el histórico.
  const rotuloPeriodo = periodo || `Año ${anio}`;
  ...
  return (
    <section className="grid gap-4">
      {onVolver && (
        <div>
          <Button variant="secondary" size="sm" onClick={onVolver}>← Volver al tablero de clientes</Button>
        </div>
      )}

      <TiraTotales columnas="sm:grid-cols-2 lg:grid-cols-4">
        <KPI titulo="Documentos vinculados" valor={`${resumen.vinculados} / ${resumen.total}`} detalle={rotuloPeriodo} tono="bueno" />
      ...
      {!grupos.length ? (
        <EmptyState
          ...
          description={`Los ${resumen.total} documentos (${rotuloPeriodo.toLowerCase()}) están vinculados a la base de clientes.`}
        />
      ) : !filtrados.length ? (
              ...
              <Card key={grupo.clave} padding="p-3.5" className="grid gap-3">
                ...
                {grupo.sugerida ? (
                  ...
                  <div className="grid gap-2">
                    ...
                    <div className="flex flex-col sm:flex-row gap-2 [&>button]:w-full sm:[&>button]:w-auto">
                      <Combobox
